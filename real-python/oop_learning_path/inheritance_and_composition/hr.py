...
class _PayrollSystem:
    def __init__(self):
        """Calculate the payrolls of employees of the company"""
    ...
    def get_policy(self, employee_id):
        ...
        if not policy:
            return ValueError(f"Invalid employee_id: {employee_id}")
        return policy

    def calculate_payroll(self, employees: list):
        print(underline("Calculating Payroll"))
        for employee in employees:
            print(f"Payroll for {employee.id_} - {employee.name}")
            print(f"- Check Amount: {float(employee.calculate_payroll())}")
            if employee.address:
                print("- Sent to:")
